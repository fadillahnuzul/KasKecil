    ...
    public function view(): View
    {
        ...
        if (Auth::user()->kk_access == 1) {
            if ($startDate and $endDate) {
                if ($company) {
                    $data_pengeluaran = Pengeluaran::with('User', 'pengajuan', 'Kategori')->where('pembebanan',$company)->where('status', 8)->whereBetween('tanggal', [$startDate, $endDate])
                                        ->where('deskripsi','!=','PENGEMBALIAN SALDO PENGAJUAN')->get();
                    $Company = Company::find($company);
                    $company = $Company->name;
                } else {
                    $data_pengeluaran = Pengeluaran::with('User', 'pengajuan', 'Kategori')->where('status', 8)->whereBetween('tanggal', [$startDate, $endDate])
                    ->where('deskripsi','!=','PENGEMBALIAN SALDO PENGAJUAN')->get();
                }
