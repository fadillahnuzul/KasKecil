    ...
    public function view(): View
    {
        ...
        if (Auth::user()->kk_access == 1) {
            if ($startDate and $endDate) {
                if ($company) {
                    $data_pengeluaran = Pengeluaran::with('User', 'pengajuan', 'Kategori')->where('pembebanan',$company)->where('status', 8)->whereBetween('tanggal_set_bkk', [$startDate, $endDate])->get();
                    $Company = Company::find($company);
                    $company = $Company->name;
                } else {
                    $data_pengeluaran = Pengeluaran::with('User', 'pengajuan', 'Kategori')->where('status', 8)->whereBetween('tanggal_set_bkk', [$startDate, $endDate])->get();
                }
            ...
            if ($startDate and $endDate) {
                if ($company) {
                    $data_pengeluaran = Pengeluaran::with('User', 'pengajuan', 'Kategori')->where('pembebanan',$company)->where('user_id', Auth::user()->id)->whereIn('status', [7,8])->whereBetween('tanggal', [$startDate, $endDate])->get();
                    $Company = Company::find($company);
                    $company = $Company->name;
                } else {
                    $data_pengeluaran = Pengeluaran::with('User', 'pengajuan', 'Kategori')->where('user_id', Auth::user()->id)->whereIn('status', [7,8])->whereBetween('tanggal', [$startDate, $endDate])->get();
                }
                ...
                if ($company) {
                    $data_pengeluaran = Pengeluaran::with('User', 'pengajuan', 'Kategori')->where('pembebanan',$company)->where('user_id', Auth::user()->id)->whereIn('status', [7,8])->get();
                    $Company = Company::find($company);
                    $company = $Company->name;
                } else {
                    $data_pengeluaran = Pengeluaran::with('User', 'pengajuan', 'Kategori')->where('user_id', Auth::user()->id)->whereIn('status', [7,8])->get();
                }
